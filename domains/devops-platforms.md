...
## Additions from Kubernetes container orchestration landscape (2026-03-18) — landscape-researcher

- Trend: **GreenOps / Carbon-aware scheduling** — Emerging DevOps sustainability discipline that integrates energy efficiency into the deployment lifecycle. Kubernetes workloads are scheduled based on real-time power grid carbon intensity data, shifting non-critical batch jobs or AI training to low-carbon regions or off-peak windows. Relevant when evaluating K8s-native platforms for enterprise sustainability requirements. Source: Gemini Deep Research (2026-03-18), 1 source.
- Term: **MLOps** — Practice of automating and standardizing the ML model lifecycle (data pipelines, training, versioning, deployment, monitoring) using infrastructure tooling. Kubeflow is the primary K8s-native MLOps framework. Relevant evaluation criterion for platforms targeting AI/ML workloads: does the platform natively support MLOps tooling or integrate with Kubeflow/similar?
- Vendor: **Kubeflow** — Open-source MLOps platform running natively on Kubernetes; manages ML data pipelines, model training, and versioned deployments at scale. Archetype: K8s extension / ML orchestration. Notable for: enabling data scientists to focus on algorithms without managing infrastructure. CNCF ecosystem.
- Vendor: **k0rdent (Mirantis)** — Unified Kubernetes control plane for managing a fleet of K8s clusters across on-premises and multi-cloud environments from a single dashboard. Archetype: Multi-cluster management / K8s fleet management. Notable for: enabling true workload portability without per-provider tooling.
- Market stat: **82% K8s production adoption** — Early 2026 figure; Kubernetes has transitioned from specialized tool to core infrastructure standard among container users. Source: Gemini Deep Research citing industry reports (2026-03-18). Use as baseline when assessing K8s-native vs. non-K8s platform adoption risk.
- Term: **Vertical Pod Autoscaler (VPA)** — K8s autoscaling mechanism that automatically adjusts CPU and memory *reservations* for individual containers (as opposed to HPA which adds/removes replicas). Distinct from horizontal autoscaling. When a platform claims "autoscaling," verify whether it is HPA-only, VPA-only, or both — VPA is required for stateful workloads where scaling-out is not viable.
- Trend: **AI-powered K8s optimization** — Platforms are beginning to use ML models to implement predictive autoscaling (scale up before traffic spike based on historical patterns) and AI-driven anomaly detection with automated remediation. Relevant as a differentiation signal for K8s-native IDP platforms targeting high-scale operations teams.

## Additions from Northflank research (2026-03-18) — solution-researcher

- New evaluation category: **GPU & AI Workload Infrastructure** — Platforms combining PaaS deployment with GPU workload support (inference, training, notebooks) represent an emerging sub-category of PaaS/IDP. Evaluate: GPU type support (NVIDIA L4/A100/H100/H200/B200), hourly/usage-based GPU pricing, co-location of GPU and CPU workloads, GPU autoscaling.
- New term: **microVM / Secure Sandbox** — Lightweight, isolated virtual machine technology (e.g., Firecracker) used to run untrusted workloads in secure multi-tenant environments. Northflank's "Secure Sandboxes" product uses microVMs to provide hardware-level isolation. Relevant to platforms offering shared-resource code execution (e.g., preview environments, CI runners) where tenant isolation is critical.
- New term: **Platform SDK** — A typed client library (e.g., TypeScript SDK) that exposes the platform's API in a language-native way with type safety. Distinct from REST API access (untyped JSON) and CLI (interactive use). Relevant for programmatic environment management and IaC-as-code workflows. Northflank explicitly gates Platform SDK behind paid tiers.
- New inference pattern: **VectorDB as managed addon** — When a platform offers pgvector as a managed PostgreSQL addon (e.g., Northflank, Supabase), infer that AI/ML similarity search workloads are in-scope for the platform. Tick "VectorDB support" in the Data Management category if pgvector is explicitly mentioned.
- Trend signal: **AI-native PaaS** — PaaS platforms are increasingly adding GPU workload support and VectorDB to their managed data services tier, positioning themselves for AI inference and RAG workloads alongside traditional web service deployments. Northflank, Railway, and Qovery are examples of this trend. When evaluating PaaS platforms, check for GPU and VectorDB as new table-stakes capabilities.
- Archetype note: **PaaS / IDP (K8s-native) — GPU variant** — Northflank specifically adds GPU PaaS (NVIDIA H100/H200/B200, microVM sandboxes) and TypeScript Platform SDK to the existing PaaS/IDP archetype. Expected strong categories now include GPU workloads and VectorDB. Update expected tick count for this archetype to ~130-155 when GPU capabilities are present.
